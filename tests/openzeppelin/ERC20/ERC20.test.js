...
const ERC20Mock = contract.fromArtifact('DummyERC20');
const OptIn = contract.fromArtifact("OptIn");

let optIn;

describe('ERC20', function () {
    const [registryFunder, initialHolder, recipient, anotherAccount] = accounts;
    ...
    beforeEach(async function () {
        optIn = await OptIn.new(anotherAccount);

        const protectedBoostableLib = await ProtectedBoostableLib.new();

        await ERC20Mock.detectNetwork();
        await ERC20Mock.link("ProtectedBoostableLib", protectedBoostableLib.address);

        this.erc1820 = await singletons.ERC1820Registry(registryFunder);
        this.token = await ERC20Mock.new(name, symbol, initialHolder, initialSupply, optIn.address);
    });
            ...
            function shouldDecreaseApproval(amount) {
                describe('when the spender had an approved amount', function () {
                    ...
                    it('emits an approval event', async function () {
                        const { logs } = await this.token.approve(spender, new BN(0), { from: initialHolder });
    ...
    describe('decrease allowance', function () {
                ...
                describe('when there was no approved amount before', function () {
                    it('reverts', async function () {
                        await expectRevert(this.token.decreaseAllowance(
                            spender, amount, { from: initialHolder }), 'ERC20-18'
                        );
                    });
                });
        ...
        describe('when the spender is the zero address', function () {
            ...
            it('reverts', async function () {
                await expectRevert(this.token.decreaseAllowance(
                    spender, amount, { from: initialHolder }), 'ERC20-13'
                );
            });
        });
    });
    ...
    describe('increase allowance', function () {
        ...
        describe('when the spender is the zero address', function () {
            ...
            it('reverts', async function () {
                await expectRevert(
                    this.token.increaseAllowance(spender, amount, { from: initialHolder }), 'ERC20-13'
                );
            });
        });
    });
    ...
    describe('_mint', function () {
        ...
        it('rejects a null account', async function () {
            await expectRevert(
                this.token.mint(ZERO_ADDRESS, amount), 'ERC20-1'
            );
        });
        ...
        describe('for a non zero account', function () {
            ...
            it('increments not totalSupply', async function () {
    ...
    describe('_burn', function () {
        describe('for a non zero account', function () {
            it('rejects burning more than balance', async function () {
                await expectRevert(this.token.burn(initialSupply.addn(1), "0x", { from: initialHolder }), 'ERC20-9'
                );
            });
            ...
            const describeBurn = function (description, amount) {
                describe(description, function () {
                    beforeEach('burning', async function () {
                        const { logs } = await this.token.burn(amount, "0x", { from: initialHolder });
                        this.logs = logs;
                    });

                    it('decrements not totalSupply', async function () {
                        expect(await this.token.totalSupply()).to.be.bignumber.equal(initialSupply);
                    });
